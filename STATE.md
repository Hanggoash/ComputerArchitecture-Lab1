...
- 已生成 macOS summary 与单平台图；当前没有 Windows raw data，因此跨平台相对性能图会安全跳过。

另有 9 条小规模冒烟测试记录保留在 `results/macos-arm64/raw/`，用于追溯构建期验证，不覆盖或篡改任何原始数据。
...
1. 在 Windows / Intel Core i5-12400F 真机上拉取同一源代码并使用 README 中的 baseline 构建命令完成正式测试。
2. 将新增的 `results/windows-x86_64/raw/` CSV 与 JSON metadata 提交到共享仓库。
3. 同时具备两平台正式 raw data 后，运行：
...
4. 将生成的 comparison summary 和 figures 用于课程实验报告，并将结论限制在规定 workload、编译配置与操作系统范围内。
